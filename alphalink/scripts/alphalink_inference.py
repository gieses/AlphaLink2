"""
AlphaLink2 inference. Turn distance constraints into models.

Example:
    $ alphalink2-inference --model_device cuda:0 --param_path params/AlphaLink-Multimer_SDA_v3.pt --data_dir msa_result --crosslinks links.pkl --bf16 --use_uniprot --save_raw_output --relax
"""
import argparse
import gzip
import json
...
from alphalink.unifold.data import protein, residue_constants, residue_constants as rc
from alphalink.unifold.data.data_ops import get_pairwise_distances
from alphalink.unifold.dataset import load_and_process, UnifoldDataset
from alphalink.unifold.modules.alphafold import AlphaFold
...
def main(args=None):
    ...
    param_loc = os.getenv("AFLINK2_PARAMS", args.param_path)

    if param_loc is None:
        print(param_loc)
        raise ValueError("AFLINK2_PARAMS is not an environment variable but also not provided")
    ...
    print("start to load params {}".format(param_loc))
    state_dict = torch.load(param_loc)["ema"]["params"]
    ...
    cur_param_path_postfix = os.path.split(param_loc)[-1]
    name_postfix = ""
    if args.sample_templates:
        name_postfix += "_st"
    ...
    for it in range(args.times):
        ...
        print("Model %d Crosslink satisfaction: %.3f Model confidence: %.3f" % (
        it, satisfied / total_xl, np.mean(out["iptm+ptm"])))
...
def inference_parser():
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="multimer_af2_crop", # default="model_5_ptm_af2"
        help="Name of the model to use (default: 'multimer_af2_crop')",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Random seed for data processing (default: 42)"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="",
        required=True,
        help="Path to data directory from the MSA script (default: '')"
    )
    parser.add_argument(
        "--crosslinks",
        type=str,
        default="",
        required=True,
        help="Path to crosslinks file"
    )
    parser.add_argument(
        "--target_name",
        type=str,
        default="",
        help="Specify the target name to enhance logging and out folder creation (default:'')"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="Specify the output directory.",
    )
    parser.add_argument(
        "--times",
        type=int,
        default=10,
        help="Number of times to run inference with different seeds (default=10)"
    )
    parser.add_argument(
        "--max_recycling_iters",
        type=int,
        default=3,
        help="Number of recycles steps (default=3)"
    )
    parser.add_argument(
        "--num_ensembles",
        type=int,
        default=1,
        help="Number of ensembles to run (default: 1)"
    )
    parser.add_argument(
        "--cutoff",
        type=float,
        default=25,
        help="Distance cutoff to count violated crosslinks (default: 25)"
    )
    parser.add_argument("--sample_templates", action="store_true", help="Sample templates for diversity")
    parser.add_argument("--use_uniprot", action="store_true",
                        default=False, help="Use uniprot MSA (default: False)")
    parser.add_argument("--relax", action="store_true",
                        default=True, help="Relax output structure (default: False)")
    parser.add_argument("--bf16", action="store_true",
                        default=False, help="Use bfloat16 to speed-up inference / lower memory usage (default: False)")
    parser.add_argument("--save_raw_output", action="store_true", help="Save raw output (default: False)")
